...
# --- CONFIGURATION FIXE DE L'AUTORITÉ 1 ---
AUTH_ID = "Auth1"
SECRETS_FILE = "secrets_Auth1.json"

intents = discord.Intents.default()
intents.message_content = True
# Ce bot répondra uniquement aux commandes commençant par "!auth1"
bot = commands.Bot(command_prefix="!auth1 ", intents=intents)
...
async def join(ctx, nom_election: str):
    """Génère la part de clé et rejoint l'élection"""
...
async def decrypt(ctx, nom_election: str):
    """Calcule les parts de déchiffrement pour chaque candidat"""
    conn = get_db_connection()
    c = conn.cursor()
    ...
    # Calcul D_i = (C1_total)^x_i pour chaque candidat
    d_parts = []
...
# Remplace par le Token du 1er bot Discord
bot.run("")